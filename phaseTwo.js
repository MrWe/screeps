...
var phaseTwo = {
	run: function(){
		...
		var extensionLength = Game.rooms[roomName].find(FIND_MY_STRUCTURES, {filter: {structureType: Game.CONSTRUCTION_SITES}}).length;
		...
		if(Game.rooms[roomName].find(FIND_MY_STRUCTURES, {filter: {structureType: STRUCTURE_EXTENSION}}).length < 5){
			console.log(Game.rooms[roomName].find(FIND_MY_STRUCTURES, {filter: {structureType: STRUCTURE_EXTENSION}}));
			buildExtensions();	
		}
...
var buildExtensions = function(){
	...
	for(var i = 0; i < 5; i++){
		console.log(Game.rooms[roomName].createConstructionSite(startSpawnx+i,startSpawny+i, STRUCTURE_EXTENSION));
